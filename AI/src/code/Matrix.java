...
public class Matrix extends SearchProblem {
	...
	Matrix(String grid) {
		...
		generatedStates.add((this.initialState));
...
//            return null;
        ...
        if (nextState == null||this.generatedStates.contains((nextState)))
			return null;
        this.generatedStates.add((nextState));
...
//					System.out.println(o);
	...
	private static MatrixSearchTreeNode IDS(MatrixSearchTreeNode root, Matrix m) {
		for(int l=0;l<Integer.MAX_VALUE;l++) {
			...
			m.generatedStates.add((m.initialState));
			Stack<MatrixSearchTreeNode> stack = new Stack<MatrixSearchTreeNode>();
			List<NeoActions> operators = Arrays.asList(NeoActions.values());
			stack.push(root);
	...
	public static void main(String[] args) {
		...
		//System.out.println("---------------------------------------------------------------");
		System.out.println(solve(grid3,"BF",false));
